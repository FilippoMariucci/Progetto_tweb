                ...
                <div class="card-body">
                    @if(isset($stats['ultime_soluzioni']) && $stats['ultime_soluzioni']->count() > 0)
                        ...
                        <div class="text-center mt-3">
                            <a href="{{ route('staff.malfunzionamenti.index') }}" class="btn btn-sm btn-outline-info">
...
$(document).ready(function() {
    ...
    // === GESTIONE TOGGLE VISTA PRODOTTI ===
    ...
    if (savedView === 'list') {
        $('#view-list').prop('checked', true).trigger('change');
    }
    ...
    // === REFRESH AUTOMATICO STATS (ogni 5 minuti) ===
    setInterval(function() {
        // Simula aggiornamento leggero dei contatori
        const shouldUpdate = Math.random() > 0.8; // 20% probabilità
        
        if (shouldUpdate) {
            console.log('📊 Statistiche aggiornate automaticamente');
            
            // Aggiorna timestamp in sidebar
            const now = new Date();
            const timeStr = now.getHours().toString().padStart(2, '0') + ':' + 
                           now.getMinutes().toString().padStart(2, '0');
            
            // Effetto visivo di aggiornamento
            $('.card-body h3').addClass('animate-pulse');
            setTimeout(() => {
                $('.card-body h3').removeClass('animate-pulse');
            }, 1000);
        }
    }, 300000); // 5 minuti
    ...
    console.log('✅ Dashboard Staff completamente funzionale');
});
